...
uint16_t args_test(uint16_t a, uint16_t b, uint16_t c);
 ...
 * this example takes 3 unsigned ints and returns one
 * they are located at:
 *    SP
 *    SP+2 a
 *    SP+4 b
...
uint16_t args_test(uint16_t a, uint16_t b, uint16_t c) {
	#asm
		mov bx, sp
		;mov ax, [bx+2]	; get uint16_t a
		;mov ax, [bx+4]	; get uint16_t b
		mov ax, [bx+6]	; get uint16_t c, returns it, it is in AX
	#endasm
}
